...
static int load_game_data(PHYSFS_file *LoadFile)
{
	...
	for (objnum_t i=object_first; i<Objects.size(); i++) {
		...
		if (Objects[i].type != OBJ_NONE) {
			...
			{
				Warning("Object %u is in non-existent segment %u, highest=%u", static_cast<unsigned>(i), static_cast<unsigned>(objsegnum), static_cast<unsigned>(Highest_segment_index));
...
int load_level(const char * filename_passed)
{
   ...
   if (Game_mode & GM_NETWORK)
	 ...
	 */
	{
		const segnum_t bad_segment(104);
		segment *segp = &Segments[bad_segment];
	if (Current_mission && !stricmp("Descent 2: Counterstrike!",Current_mission_longname) && !stricmp("d2levc-4.rl2",filename)
		&& ( Vertices[segp->verts[0]].x == -53990800 && Vertices[segp->verts[0]].y == -59927741 && Vertices[segp->verts[0]].z == 23034584 )
		&& ( segp->sides[1].normals[0].x == 56775 && segp->sides[1].normals[0].y == -27796 && segp->sides[1].normals[0].z == -17288 && segp->sides[1].normals[1].x == 50157 && segp->sides[1].normals[1].y == -34561 && segp->sides[1].normals[1].z == -24180 )
		&& ( segp->sides[2].normals[0].x == 60867 && segp->sides[2].normals[0].y == -19485 && segp->sides[2].normals[0].z == -14507 && segp->sides[2].normals[1].x == 55485 && segp->sides[2].normals[1].y == -29668 && segp->sides[2].normals[1].z == -18332 )
		)
	{
			Vertices[segp->verts[0]].x = -53859726;
			Vertices[segp->verts[0]].y = -59927743;
			Vertices[segp->verts[0]].z = 23034586;
			segp->sides[1].normals[0].x = 56123;
			segp->sides[1].normals[0].y = -27725;
			segp->sides[1].normals[0].z = -19401;
			segp->sides[1].normals[1].x = 49910;
			segp->sides[1].normals[1].y = -33946;
			segp->sides[1].normals[1].z = -25525;
			segp->sides[2].normals[0].x = 60903;
			segp->sides[2].normals[0].y = -18371;
			segp->sides[2].normals[0].z = -15753;
			segp->sides[2].normals[1].x = 57004;
			segp->sides[2].normals[1].y = -26385;
			segp->sides[2].normals[1].z = -18688;
			// I feel so dirty now ...
	}
	}
...
int create_new_mine(void)
{
	...
	med_create_segment(&Segments[segment_first],0,0,0,DEFAULT_X_SIZE,DEFAULT_Y_SIZE,DEFAULT_Z_SIZE,&m1);
