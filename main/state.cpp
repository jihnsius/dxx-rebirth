...
// Save the control cen info
	...
	PHYSFS_write(fp, &Control_center_present, sizeof(int), 1);
	{
		...
		PHYSFS_write(fp, &dead_controlcen_object_num, sizeof(dead_controlcen_object_num), 1);
	}
...
// Save the automap visited info
	...
	{
		PHYSFS_write(fp, &Automap_visited[segment_first], sizeof(Automap_visited[segment_first]), Highest_segment_index + 1);
	}
	else
		PHYSFS_write(fp, &Automap_visited[segment_first], sizeof(Automap_visited[segment_first]), MAX_SEGMENTS_ORIGINAL);
...
// Restore the cheats enabled flag
	...
	if ( Highest_segment_index+1 > MAX_SEGMENTS_ORIGINAL )
	{
		...
		PHYSFS_read(fp, &Automap_visited[segment_first], sizeof(Automap_visited[segment_first]), Highest_segment_index + 1);
	}
	else
		PHYSFS_read(fp, &Automap_visited[segment_first], sizeof(Automap_visited[segment_first]), MAX_SEGMENTS_ORIGINAL);
