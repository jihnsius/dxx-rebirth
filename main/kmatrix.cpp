...
static int kmatrix_handler(window *wind, d_event *event, kmatrix_screen *km)
{
	...
	switch (event->type)
	{
		...
		case EVENT_MOUSE_DOUBLE_CLICKED:
		...
		case EVENT_UI_DIALOG_DRAW:
		case EVENT_UI_GADGET_PRESSED:
		case EVENT_UI_LISTBOX_MOVED:
		case EVENT_UI_LISTBOX_SELECTED:
		case EVENT_UI_USERBOX_DRAGGED:
			break;
	}
