...
static const char *const Escort_goal_text[] = {
	"BLUE KEY",
	"YELLOW KEY",
	"RED KEY",
	"REACTOR",
...
void detect_escort_goal_accomplished(objnum_t index)
{
	if (!Buddy_allowed_to_talk)
		return;
...
void set_escort_special_goal(int special_key)
{
	...
	if ( Looking_for_marker != -1 ) {
		Escort_special_goal = (escort_goal_t)(ESCORT_GOAL_MARKER1 + marker_key - KEY_1);
	} else {
		switch (special_key) {
...
static void escort_go_to_goal(dxxobject *objp, ai_static *aip, segnum_t goal_seg)
{
