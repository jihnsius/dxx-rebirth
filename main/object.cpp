...
void init_objects()
{
	...
	for (j=segment_first;j<Segments.size();j++)
...
void dead_player_frame(void)
{
	...
	if (Player_is_dead)
	{
		...
		if (time_dead > DEATH_SEQUENCE_EXPLODE_TIME) {
			if (!Player_exploded) {
				py_notify_player_ship_destroyed();
			...
			if (Players[Player_num].hostages_on_board > 1)
				...
				HUD_init_message(HM_DEFAULT, "%s", TXT_SHIP_DESTROYED_1);
			else
				HUD_init_message(HM_DEFAULT, "%s", TXT_SHIP_DESTROYED_0);
