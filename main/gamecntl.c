...
static int do_game_pause()
{
	...
	snprintf(msg, 1024, "PAUSE\n\nSkill level:  %s\nHostages on board:  %d%cTime on level: %s\nTotal time in game: %s", MENU_DIFFICULTY_TEXT(Difficulty_level), Players[Player_num].hostages_on_board, (Newdemo_state!=ND_STATE_PLAYBACK) ? '\n' : 0, level_time, total_time);
...
static void select_big_window_function(unsigned which)
{
	if (which >= N_players)
		which = Player_num;
	const objnum_t objnum = Players[which].objnum;
...
void do_cheat_menu()
{
	...
	nm_set_item_checkbox(&mm[0],"Invulnerability",Players[Player_num].flags & PLAYER_FLAGS_INVULNERABLE);
	nm_set_item_checkbox(&mm[1],"Cloaked",Players[Player_num].flags & PLAYER_FLAGS_CLOAKED);
