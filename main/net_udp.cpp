	...
	switch (event->type)
	{
		...
		case EVENT_NEWMENU_SELECTED:
		{
			if (((citem+(NLPage*UDP_NETGAMES_PPAGE)) >= 4) && (((citem+(NLPage*UDP_NETGAMES_PPAGE))-4) <= num_active_udp_games-1))
			{
				...
				dj->start_time = timer_query();
				dj->last_time = 0;
				Netgame.players[0].protocol.udp.addr = dj->host_addr = Active_udp_games[(citem+(NLPage*UDP_NETGAMES_PPAGE))-4].game_addr;
				dj->connecting = 1;
				return 1;
			}
			else
...
static void net_udp_send_markers()
 {
...
int net_udp_setup_game()
{
	...
	nm_set_item_radio(&m[optnum], "Open game",(!Netgame.RefusePlayers && !Netgame.game_flags & NETGAME_FLAG_CLOSED),1); optnum++;
	opt.closed = optnum;
	nm_set_item_radio(&m[optnum], TXT_CLOSED_GAME,Netgame.game_flags & NETGAME_FLAG_CLOSED,1); optnum++;
	opt.refuse = optnum;
...
static int show_game_rules_handler(window *wind, d_event *event, netgame_info *netgame)
{
	...
	switch (event->type)
	{
		...
		case EVENT_WINDOW_DRAW:
			...
			gr_string( FSPACX(275),FSPACY( 65), netgame->game_flags & NETGAME_FLAG_SHOW_MAP?"ON":"OFF");
