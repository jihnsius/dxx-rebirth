...
int gr_set_mode(u_int32_t mode)
{
	...
	unsigned char *gr_bm_data=(unsigned char *)d_realloc(grd_curscreen->sc_canvas.cv_bitmap.bm_data, w*h);//since we use realloc, we want to keep this pointer around.
	if (!gr_bm_data)
		return 0;
	...
	gr_init_canvas(&grd_curscreen->sc_canvas, gr_bm_data, BM_OGL, w, h);
