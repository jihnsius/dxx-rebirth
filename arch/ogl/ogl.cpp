...
static void ogl_texture_stats(void)
{
	...
	gr_printf(FSPACX(2),FSPACY(1),"%i flat %i tex - sprites %i bitmaps",r_polyc,r_tpolyc,r_bitmapc);
...
void ogl_cache_level_textures(void)
{
	...
	for (ef=0;ef<max_efx;ef++){
		...
		for (segnum_t seg=segment_first;seg<Num_segments;++seg){
...
void ogl_init_pixel_buffers(int w, int h)
{
	...
	if (texbuf)
		d_free(texbuf);
	MALLOC(texbuf, GLubyte, max(w, 1024)*max(h, 256)*4);
