...
static int read_m3u(void)
{
	...
	while (buf < JukeboxSongs.list_buf + length - 1)
	{
		...
		if (*buf != '#')	// ignore comments / extra info
		{
			...
			{
				char **new_list = (char **) d_realloc(JukeboxSongs.list, JukeboxSongs.max_buf*sizeof(char *)*MEM_K);
				if (new_list == NULL)
					break;
...
int jukebox_play()
{
	...
	const unsigned long size_full_filename = strlen(GameCfg.CMLevelMusicPath)+strlen(music_filename)+1;
