...
static void set_average_light_on_all_fast(void)
{
	...
	fix	al;
	int	alc;
	segnum_t	seglist[MAX_LIGHT_SEGS];
	segnum_t	s, *segptr;
...
static void assign_uvs_to_side(segment *segp, int sidenum, uvl *uva, uvl *uvb, int va, int vb)
{
	...
	if (mag01 < F1_0/1024 )
		editor_status("U, V bogosity in segment #%i, probably on side #%i.  CLEAN UP YOUR MESS!", static_cast<unsigned>(segp-Segments), sidenum);
	else {
