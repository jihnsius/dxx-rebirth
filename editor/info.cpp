...
static const char * get_object_type(int num, char *name)
{
	switch (num) {
...
static const char * get_control_type(int num, char *name)
{
	switch (num) {
...
static const char * get_movement_type(int num, char *name)
{
	switch (num) {
...
static const char * get_ai_behavior(int num, char *name)
{
...
static void info_display_default(int show_all)
{
	...
	if (( old_Cursegp_num != Cursegp-Segments ) || ( old_Curside != Curside ))	{
		gr_uprintf( 0, 48, "Cursegp/side: %3hu/%1d", static_cast<unsigned short>(Cursegp-Segments), Curside);
